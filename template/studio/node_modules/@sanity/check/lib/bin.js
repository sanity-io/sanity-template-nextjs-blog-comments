...
var productionMode = process.env.NODE_ENV === 'production' || includes(args, '-p') || includes(args, '--production');
...
fse.readJson(manifestDir).catch(err => {
  console.error(chalk.red("".concat(tag, " Failed to read \"").concat(manifestDir, "\":\n").concat(err.message)));
  process.exit(1);
}).then(() => sanityCheck({
  dir,
  productionMode
})).catch(err => {
  console.error(chalk.red(err.sanityCheck ? "".concat(tag, "\n\n").concat(err.message) : err.stack));
  process.exit(1);
});