...
function createAttributes(id, document, extraEvents) {
  var attributes = null;

  if (document) {
    attributes = _objectSpread({}, document);
    delete attributes._rev;
  }

  if (extraEvents) {
    var _iterator = _createForOfIteratorHelper(extraEvents),
        _step;

    try {
      for (_iterator.s(); !(_step = _iterator.n()).done;) {
        var evt = _step.value;
        var effect = evt.effects[id].apply;

        if (effect) {
          attributes = (0, _mendoza.applyPatch)(attributes, effect);
        }
      }
    } catch (err) {
      _iterator.e(err);
...
class Timeline {
  ...
  findLastPublishedBefore(chunk) {
    for (var chunkIdx = chunk ? chunk.index - 1 : this._chunks.lastIdx; chunkIdx >= this._chunks.firstIdx; chunkIdx--) {
      ...
      if (_chunk3.type === 'publish' || _chunk3.type === 'initial') {
        return _chunk3;
      }
    }
  ...
  chunkByTransactionIndex(idx) {
    ...
    for (;;) {
      ...
      if (!_chunk4) throw new Error('transaction does not belong in any chunk');

      if (idx >= _chunk4.end) {
        chunkIdx++;
      } else if (idx < _chunk4.start) {
        chunkIdx--;
      } else {
        return _chunk4;
      }
  ...
  calculateDiff(initialDoc, finalDoc, firstIdx, lastIdx) {
    ...
    for (var chunkIdx = firstIdx; chunkIdx <= lastIdx; chunkIdx++) {
      var _chunk5 = this._chunks.get(chunkIdx);

      if (!firstChunk) firstChunk = _chunk5;

      for (var _idx6 = _chunk5.start; _idx6 < _chunk5.end; _idx6++) {
