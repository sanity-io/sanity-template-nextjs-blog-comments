 ...
 * for instance.
 */
var klawPath = require.resolve('klaw-sync');

var resolvePath = require.resolve('resolve-from');

var hookPath = __dirname;
var registered = false;

var requireContext = (directory, recursive, regExp) => {
  ...
  try {
    ...
    var klaw = require(klawPath);

    var resolveFrom = require(resolvePath); // We need to resolve `./foo` from where the _caller_ is situated
...
function register() {
  ...
  try {
    module.constructor.wrap = script => {
      var requireContextScript = requireContext.toString().replace(/hookPath/g, JSON.stringify(hookPath)).replace(/klawPath/g, JSON.stringify(klawPath)).replace(/resolvePath/g, JSON.stringify(resolvePath)); // Assigning the function may very well crash in an upcoming version,
      // so try to catch and warn if this is the case

      return wrap("try {\n        require.context = ".concat(requireContextScript, "\n      } catch (err) {\n        console.warn('Error assigning require.context function:\\n' + err.message)\n      }\n\n").concat(script));
    };
  } catch (err) {
